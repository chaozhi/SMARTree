...
BeginPackage["SMARTree`",{"Toolbox`","Metropolis`","RandomGenerator`","Genealogy`CoalescentTree`","Genealogy`LocalTrees`","SMARTree`Probability`"}]

initializeMCMC::usage = "initializeMCMC  "

savetofile::usage = "savetofile  "

updatechain::usage = "updatechain  "

swapchain::usage = "swapchain  "

inftoscreen::usage = "infortoscreen  "

logpritree::usage = "logpritree  "

logpriSNP::usage = "logpriSNP "

randommidtree::usage = "randommidtree  "

caltype1ff::usage = "caltype1ff  "

updateThetaEpsilon::usage = "updateThetaEpsilon  "

updateRho::usage = "updateRho  "

updatetimescale::usage = "updatetimescale  "

rjupdatetreels::usage = "rjupdatetreels  "

updatetreepos::usage = "updatetreepos  "

segloglike::usage = "segloglike  "

updatetreetree::usage = "updatetreetree  "

updatetreetree2::usage = "updatetreetree2  "

updatetreeinsert::usage = "updatetreeinsert  "

updatetreedelete::usage = "updatetreedelete  "

updatecoaltime::usage = "updatecoaltime  "

updatehyperscale::usage = "updatehyperscale  "

updatehypershift::usage = "updatehypershift  "

updateshift::usage = "updateshift  "

updatehypertree::usage = "updatehypertree  "

updatetreeswapch::usage = "updatetreeswapch  "

updatetreepostree::usage = "updatetreepostree  "

updatetreedelete2::usage = "updatetreedelete2  "

updatetreeinsert2::usage = "updatetreeinsert2  "

updatetreechecker::usage = "updatetreechecker  "

blockupdatetreepos::usage = "blockupdatetreepos  "

blockupdatetreetree::usage = "blockupdatetreetree  "

blockupdatetreepostree::usage = "blockupdatetreepostree  "

updatesegtreetree::usage = "updatesegtreetree  "

randomjumpsegcondtree::usage = "randomjumpsegcondtree  "

updatesegtreeinsert::usage = "updatesegtreeinsert  "

updatesegtreedelete::usage = "updatesegtreedelete  "

updatesegtreepos::usage = "updatesegtreepos  "

randomjumptree::usage = "randomjumptree  "

logpdfjumptree::usage = "logpdfjumptree  "

jumptreeprob::usage = "jumptreeprob  "

randomjumpcondtree::usage = "randomjumpcondtree  "

mapnode::usage = "mapnode  "

condTreeTransitionTypeII::usage = "condTreeTransitionTypeII  "

swapchain::usage = "swapchain  "

permutechain::usage = "permutechain  "
...
initializeMCMC[inputsnpls_,nsq_, nbp_,prithetashape_,prithetascale_,prithetamin_, prithetamax_, prilambdamin_,prilambdamax_, priepsmin_,priepsmax_] :=
    Module[ {snpls = inputsnpls,lambda,theta, epsilon, rho, treels, adpscale, adpthetaerr,adphyperscale,adpshift,adpposseglen,adprho,
        adphypershift,indtr,temp,treels2,treemap,ytree},
        theta = RandomReal[If[ prithetascale == Infinity,
                               UniformDistribution[{prithetamin, 0.001}],
                               TruncatedDistribution[{prithetamin, prithetamax},GammaDistribution[prithetashape, 0.001 prithetashape]]
                           ]];
        epsilon = (priepsmin+priepsmax)/2;
        rho = 0.0004;
        lambda = rho/theta;
        ...
        snpls[[All, -1]] = TreeLogLikelihood[snpls[[All, 2]], ytree, snpls[[All, 3]], theta, epsilon];
...
savetofile[mcstate_, nsq_,adpheat_,outputconcise_, outputcold_,isheating_] :=
    Module[ {mcstate2, mcstate3,i},
        mcstate2 = Flatten[mcstate, 1][[All, ;; 5]];
        Do[
          mcstate2[[i, 5]] = Prepend[mcstate2[[i, 5]], adpheat];
          mcstate2[[i, 4]] = Length[mcstate2[[i, 4]]], {i, Length[mcstate2]}];
        mcstate2[[All, 5, ;; -3]] = mcstate2[[All, 5, ;; -3, {4, 5}]];
        PutAppend[mcstate2, outputconcise];
        mcstate3 = If[ isheating,
                       First[Select[#[[All, ;; 5]], First[#] == 1 &]] & /@ mcstate,
                       mcstate[[All,1]]
                   ];
        Do[
          mcstate3[[i, 5]] = Prepend[mcstate3[[i, 5]], adpheat];
          mcstate3[[i, 4, All, 2]] = {#[[1, nsq ;;]], TreeToAdjacencyList[#]}&/@mcstate3[[i, 4, All, 2]], {i, Length[mcstate3]}];
        mcstate3[[All, 5, ;; -3]] = mcstate3[[All, 5, ;; -3, {4, 5}]];
...
testsnpls[snpls_,treels_,nbp_,theta_,epsilon_,msg_] :=
    Module[ {ls,trpos,temp},
        ls = Append[treels[[All, 1]], nbp + 1];
        trpos = IndexByInterpolation[ls][snpls[[All, 1]]];
        temp = TreeLogLikelihood[snpls[[All, 2]], treels[[trpos, 2]],snpls[[All, 3]], theta, epsilon];
...
updatechain[chainstate_, nsq_, nbp_, prithetashape_,prithetascale_, prithetamin_,prithetamax_,prilambdamin_,prilambdamax_,priepsalpha_, 
    priepsbeta_,priepsmin_,priepsmax_,isadp_,adprate_,rjfreq_,isprint_] :=
    Module[ {heat,logl, theta, epsilon,rho, treels, adpscale, adprho,adpthetaerr,adphyperscale,adpshift,adphypershift,
        adpposseglen, segarlist,indtr, deltd,snpls,direct,istest},
        {heat, logl, {theta, epsilon,rho},treels,{adprho,adpthetaerr,adpscale,adphyperscale,adpshift,adphypershift,
            adpposseglen,segarlist,indtr},snpls} = chainstate;
        ...
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"0:"<>ToString[heat]]
            ];
        ];
        If[ isprint,
            Print["update rj..."]
        ...
        {snpls,treels,adpposseglen,segarlist,indtr} = rjupdatetreels[snpls, treels, theta, epsilon, nsq,nbp, 
            prilambdamin,prilambdamax,heat, rjfreq,isadp,adprate,adpposseglen];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"1: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
            Print["update timescale..."]
        ];
        {snpls,treels, adpscale} = updatetimescale[snpls, treels, theta, epsilon, nsq, nbp,prilambdamin,prilambdamax, 
            heat, isadp, adprate, adpscale];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"2: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
            Print["update timeshift..."]
        ];
        {snpls,treels, adpshift} = updateshift[snpls, treels, theta, epsilon, nsq, nbp,prilambdamin,prilambdamax, 
            heat, isadp, adprate, adpshift];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"3: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
        ...
        {snpls,{theta, epsilon}, adpthetaerr} = updateThetaEpsilon[snpls, treels, theta, epsilon,rho,nbp, 
            prithetashape,prithetascale, prithetamin,prithetamax,priepsalpha, priepsbeta,priepsmin,priepsmax,prilambdamin,prilambdamax,
            heat, isadp, adprate,adpthetaerr];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"4: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
        ...
        {snpls, treels, {theta, epsilon}, adphyperscale} = updatehyperscale[snpls, treels, theta, epsilon, nsq, nbp, 
             prilambdamin,prilambdamax, prithetashape,prithetascale,prithetamin, prithetamax,priepsalpha, priepsbeta, priepsmin,priepsmax,
             heat, isadp, adprate, adphyperscale];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"5: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
        ...
        {snpls, treels, {theta, epsilon}, adphypershift} = updatehypershift[snpls, treels, theta, epsilon, nsq, nbp, 
            prilambdamin,prilambdamax, prithetashape,prithetascale,prithetamin, prithetamax,priepsalpha, priepsbeta, priepsmin,priepsmax,
            heat, isadp, adprate, adphypershift];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"6: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
            Print["update rho..."];
        ...
        logl+=(priepsalpha-1) Log[epsilon]+(priepsbeta-1) Log[1-epsilon]+
                (prithetashape-1)Log[theta]-theta/prithetascale-Log[theta];
        deltd = Append[Differences[treels[[All, 1]]], nbp - treels[[-1, 1]]];
        logl+=(Length[treels] - 1) Log[rho/2]-(deltd.treels[[All, 3]]) rho/2;
        logl+=logpriSNP[snpls[[All,1]],treels, 1, Length[treels], theta,epsilon, nbp];
        If[ direct == -1,
            {snpls, treels} = reversechromosome[snpls, treels, nbp]
        ];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"7: "<>ToString[heat]]
            ];
        ];
        {heat, logl, {theta, epsilon,rho},treels, {adprho,adpthetaerr,adpscale,adphyperscale,adpshift,adphypershift,adpposseglen,segarlist,indtr},snpls}
    ]
    
permutechain[inputmcstate_, nbp_,isadp_, adprate_, inputadpset_,size_] :=
...
swapchain[inputgroupmcstate_, nbp_, isadp_, adprate_,inputadpset_,size_] :=
    Module[ {groupmcstate = inputgroupmcstate, adpset = inputadpset, ii,k, ar, ind},
        ind = Table[0,{size}];
        Do[
         ii = RandomSample[Range[Length[groupmcstate]], 2];
         ar = groupmcstate[[Reverse[ii], 1]].groupmcstate[[ii, 2]]-groupmcstate[[ii, 1]].groupmcstate[[ii, 2]];
         ar = Min[1, Exp[ar]];
         ind[[k]] = ar;
         If[ RandomReal[] < ar,
...
updateThetaEpsilon[inputsnpls_,treels_,inputtheta_, inputepsilon_, rho_,nbp_,prithetashape_, 
    prithetascale_,prithetamin_,prithetamax_,priepsalpha_,priepsbeta_,priepsmin_,priepsmax_,prilambdamin_,prilambdamax_,
    ...
    Module[ {snpls = inputsnpls,adpset = inputadpset,theta = inputtheta,epsilon = inputepsilon,
        ...
        logprihyper = Function[{x},
            -Log[x[[1]]]+(prithetashape-1)Log[x[[1]]]-x[[1]]/prithetascale + 
        ...
        Do[
          ...
          If[ prithetamin<propx[[1]]<prithetamax && priepsmin < propx[[2]] <priepsmax,
              ...
              If[ propsnpls==={},
                  ar = 0,
                  propsnpls[[All, -1]] = TreeLogLikelihood[propsnpls[[All, 2]], ytree, propsnpls[[All, 3]],propx[[1]], propx[[2]]];
                  ar = Total[propsnpls[[All, -1]]] - Total[snpls[[All, -1]]];
              ];
              ar += logprihyper[propx] - logprihyper[{theta, epsilon}];
              ar+= logpriSNP[snpls[[All,1]],treels, 1, Length[treels], propx[[1]],propx[[2]], nbp]-logpriSNP[snpls[[All,1]],treels, 1, Length[treels], theta,epsilon, nbp];
              ar = heat ar+logpritree[treels, nbp, propx[[1]],prilambdamin,prilambdamax,heat]-
                          logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
              ar = Min[1, Exp[ar]];
              If[ RandomReal[] < ar,
                  {theta, epsilon} = propx;
                  snpls = propsnpls;
...
updateRho[treels_, rho_, nbp_, theta_,prilambdamin_,prilambdamax_,heat_,isadp_, adprate_, adprho_, heat_] :=
    Module[ {deltd,a,b,logpdf, res},
        deltd = Append[Differences[treels[[All, 1]]], nbp - treels[[-1, 1]]];
        a = heat (Length[treels]-1);
        b = heat (Total[deltd treels[[All, 3]]]/2);
        logpdf = Function[{x}, a Log[x] - x b];
        res = AdaptiveMetropolis[logpdf, rho, 1, 1, Boole[isadp], adprate, adprho, DomainConstraint -> (theta prilambdamin<#<theta prilambdamax &)];
        res[[{1, -1}]]
    ]    

logpritree[treels_, nbp_, theta_,prilambdamin_,prilambdamax_,heat_] :=
    Module[ {deltd, a,b,up,low,res},
        deltd = Append[Differences[treels[[All, 1]]], nbp-treels[[-1, 1]]];
        a = heat (Length[treels]-1);
        b = heat (Total[deltd treels[[All, 3]]]/2);
        low = theta prilambdamin;
        up = theta prilambdamax;
        res = -a Log[2.]-(a+1) Log[b];
        res+=If[ up> (a+1)/b,
                 Log[Gamma[a+1]-Gamma[a+1,up b]-Gamma[a+1,0,low b]],
                 Log[Gamma[a+1,low b,up b]]
             ]
    ]        
    
logpritbl[treels_, nbp_, theta_] :=
    Module[ {forest,nleaf,tbl,set},
        forest = treels[[All, 2]];
...
logpriSNP[snpx_,treels_, istart_, iend_, theta_, epsilon_, nbp_] :=
    Module[ {nleaf, dls, forest, lam, set1, pos, set11, temp, lam11, set2,i,lam2, temp2, pls},
        If[ snpx==={},
            0,
            nleaf = LeafNumber[treels[[istart, 2]]];
            ...
            pls = temp[[1]] Total[temp2[[;; 2]]] + temp[[2]] temp2[[1]] + 
              temp[[2]] temp2[[2]]/nleaf + temp[[2]] temp2[[3]] (nleaf - 1) + 
              temp[[3]] temp2[[2]] 2/nleaf + temp[[3]] temp2[[3]] + 
              temp[[3]] temp2[[4]] (nleaf - 2);        
            (*pls=Exp[-TotalBranchHeight[forest] theta/2];*)
            dls.Log[pls]
        ]
    ]
      

updatetimescale[snpls_,treels_, theta_, epsilon_,nsq_,nbp_,prilambdamin_, prilambdamax_,
    ...
    Module[ {dim,props,proptreels,propsnpls,ls,map,trees,ar,adpset = inputadpset},
        ...
        If[ propsnpls==={},
            ...
            propsnpls[[All, -1]] = TreeLogLikelihood[propsnpls[[All, 2]], trees, propsnpls[[All, 3]],theta, epsilon];
            ar = Total[propsnpls[[All, -1]]] - Total[snpls[[All, -1]]];
        ];
        ar += (Total[proptreels[[All, -1]]] + Log[Times @@ proptreels[[;; -2, 3]]])-
                (Total[treels[[All, -1]]] + Log[Times @@ treels[[;; -2, 3]]]);
        ar+= logpriSNP[snpls[[All,1]],proptreels, 1,Length[proptreels], theta, epsilon, nbp]-logpriSNP[snpls[[All,1]],treels, 1,Length[treels], theta, epsilon, nbp];
        ar = heat ar+logpritree[proptreels, nbp, theta, prilambdamin,prilambdamax,heat]-
                logpritree[treels, nbp, theta, prilambdamin,prilambdamax,heat];
...
updateshift[snpls_, treels_, theta_, epsilon_, nsq_, nbp_,prilambdamin_,prilambdamax_, 
  heat_, isadp_, adprate_, inputadpset_] :=
...
updatehyperscale[inputsnpls_, inputtreels_, inputtheta_, 
   inputepsilon_, nsq_,nbp_, prilambdamin_,prilambdamax_,prithetashape_,
   ...
   heat_, isadpt_, adprate_, inputadpset_] :=
    Module[ {adpset = inputadpset, snpls = inputsnpls, 
      ...
      propsnpls, ar,i},
        logprihyper = Function[{x},
            -Log[x[[2]]]+(prithetashape-1) Log[x[[2]]]-x[[2]]/prithetascale + 
         ...
         If[ prithetamin<propx[[2]] <prithetamax &&priepsmin < propx[[-1]] < priepsmax,
             ...
             If[ propsnpls === {},
                 ...
                 propsnpls[[All, -1]] = TreeLogLikelihood[propsnpls[[All, 2]], proptreels[[trpos, 2]],
                     propsnpls[[All, 3]], propx[[2]], propx[[3]]];
                 ar = Total[propsnpls[[All, -1]]] - Total[snpls[[All, -1]]];
             ];
             ar += (Total[proptreels[[All, -1]]] +Log[Times @@ proptreels[[;; -2, 3]]])-
                     (Total[treels[[All, -1]]] + Log[Times @@ treels[[;; -2, 3]]]);
             ar += logprihyper[propx] - logprihyper[{logmtbh, theta, epsilon}];
             ar+= logpriSNP[snpls[[All,1]],proptreels, 1,Length[proptreels], propx[[2]],propx[[3]], nbp]-logpriSNP[snpls[[All,1]],treels, 1,Length[treels], theta, epsilon, nbp];
             ar = heat ar+logpritree[proptreels, nbp, propx[[2]],prilambdamin,prilambdamax,heat]-
                     logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
...
updatehypershift[inputsnpls_, inputtreels_, inputtheta_, 
   inputepsilon_, nsq_, nbp_, prilambdamin_,prilambdamax_,prithetashape_,
   prithetascale_,prithetamin_,prithetamax_, priepsalpha_, priepsbeta_, priepsmin_,priepsmax_, heat_, 
...
segloglike[inputsnpls_, tree_, xstart_, xend_, theta_, epsilon_] :=
...
(*type = TreeTransitionTypeII[tree1, tree2]*)    
        ...
        Switch[First[type],
            ...
            Switch[RandomChoice[{p00, p00, 1 - 2 p00} -> {1, 2, 3}],
                ...
                {Switch[RandomChoice[ff->Range[4]],
                    1,
                    NestWhile[NextLocalTree[#, RandomRecombination[#, e00]]&,tree1,SameCoalescentTreeQ[tree1, #]&],
                    2,
                    NextLocalTree[tree1, RandomRecombination[tree1, {{v12, v10}, -1, {v11, v10}}]],
                    3,
                    NextLocalTree[tree1, RandomRecombination[tree1, {{v12, v10}, -1, {v10, v13}}]],
                    4,
                    NextLocalTree[tree1, RandomRecombination[tree1, {{v12, v10}, -1, {v32, v13}}]]                                       
                ],type,ff}
            ],
            2,
            Which[
                ...
                {NextLocalTree[tree1, RandomRecombination[tree1, op]],type,ff},
...
randommidtree[tree1_, tree2_] :=
    randommidtree[tree1, tree2,TreeTransitionTypeII[tree1, tree2]]

updatecoaltime[snpls_, treels_, theta_, epsilon_, nsq_, nbp_,prilambdamin_,prilambdamax_, heat_] :=
    Module[ {ct,temp,minpos,maxpos,coalt2,propct,proptreels,propsnpls,ar,ar2},
        ...
        {propsnpls, ar} = segloglike2[snpls, proptreels, minpos, maxpos, theta, epsilon,nbp];
        ...
        ar+= logpriSNP[snpls[[All,1]],proptreels, minpos, maxpos, theta, epsilon, nbp]-logpriSNP[snpls[[All,1]],treels, minpos, maxpos, theta, epsilon, nbp];
        ar = heat ar +logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax,heat]-
            logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
        ar2 = If[ ct == coalt2[[-2]],
                  (-2 propct) - (-2 ct),
...
updatetreepos[snpls_,treels_, foc_,theta_, epsilon_, nbp_, prilambdamin_,prilambdamax_,heat_] :=
    Module[ {ar,s1, s2, s3, props, proptreels,propsnpls,tree1,tree2},
        If[ Length[treels] == 1,
            ...
            {propsnpls,ar} = If[ s2 < props,
                                 segloglike[snpls, tree1, s2, props, theta, epsilon],
                                 segloglike[snpls, tree2, props,s2, theta, epsilon]
                             ];
            ar = heat ar+logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat] - 
               logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
            ar = Min[1, Exp[ar]]
...
updatetreepostree[snpls_,treels_,foc_,theta_, epsilon_, nbp_, prilambdamin_,prilambdamax_,heat_] :=
    Module[ {s1, tree1, proptr, proptreels,propsnpls,ar, ar2, s2, tree2, s3,tree3, type, ff,direct,xleft,xright,
        ...
        Which[
         ...
         {propsnpls,ar} = segloglike[snpls, proptr, s1, nbp + 1, theta, epsilon];
         ar = heat ar+ logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax,heat]-
                 logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
         ...
         If[ SameCoalescentTreeQ[tree1,proptr],
             ...
             {propsnpls,ar} = segloglike[snpls, proptr, s1,s2,  theta, epsilon];
         ...
         If[ props2<=s2,
             If[ !SameCoalescentTreeQ[tree2,proptr],
                 {propsnpls,ar} = segloglike[propsnpls,proptr,props2,nbp+1,theta,epsilon],
                 {propsnpls,ar} = segloglike[propsnpls,proptr,props2,s2,theta,epsilon]
             ],
             {propsnpls,ar} = segloglike[propsnpls,tree1,s2,props2,theta,epsilon];
             If[ !SameCoalescentTreeQ[tree2,proptr],
                 {propsnpls,temp} = segloglike[propsnpls,proptr,props2,nbp+1,theta,epsilon];
                 ar+=temp;
             ];
         ];
         ...
         If[ SameCoalescentTreeQ[treels[[foc - 1, 2]], treels[[foc + 1, 2]]],
             ...
             propsnpls[[pos, -1]] = TreeLogLikelihood[propsnpls[[pos, 2]], trees, propsnpls[[pos, 3]],theta, epsilon];
             ...
             If[ props2<=s2,
                 If[ !SameCoalescentTreeQ[tree2,proptr],
                     {propsnpls,ar} = segloglike[propsnpls,proptr,props2,s3,theta,epsilon],
                     {propsnpls,ar} = segloglike[propsnpls,proptr,props2,s2,theta,epsilon]
                 ],
                 {propsnpls,ar} = segloglike[propsnpls,tree1,s2,props2,theta,epsilon];
                 If[ !SameCoalescentTreeQ[tree2,proptr],
                     {propsnpls,temp} = segloglike[propsnpls,proptr,props2,s3,theta,epsilon];
                     ar+=temp;
                 ];
             ];
             ar+=(Total[proptreels[[foc ;; foc + 1, -1]]] +Log[proptreels[[foc, 3]]])-
...
updatetreetree[snpls_,treels_,foc_,theta_, epsilon_, nbp_, prilambdamin_,prilambdamax_,heat_] :=
    Module[ {s1, tree1, proptr, proptreels,propsnpls,ar, ar2, s2, tree2, s3,tree3, type, ff,direct,xleft,xright,i2min,
        ...
        Which[
         ...
         If[ SameCoalescentTreeQ[treels[[1, 2]], proptr],
             ...
             proptreels = treels;
             proptreels[[1, 2 ;;]] = {proptr, TotalBranchHeight[proptr], TreeLogPriorProb[proptr]};
             {propsnpls, ar} = segloglike[snpls, proptr, treels[[1, 1]], nbp + 1, theta,epsilon];
             ar = heat ar +logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax,heat] - 
               logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
         ...
         If[ SameCoalescentTreeQ[treels[[foc,2]],proptr],
             ...
             {propsnpls,ar} = segloglike[snpls, proptr, treels[[foc,1]],treels[[foc+1,1]],  theta, epsilon];
             ar += (Total[proptreels[[foc ;; foc + 1, -1]]] + Log[proptreels[[foc, 3]]])-
                  (Total[treels[[foc ;; foc + 1, -1]]] +Log[treels[[foc, 3]]]);
             ar = heat ar+logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax,heat]-
                     logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
         ...
         If[ SameCoalescentTreeQ[treels[[foc, 2]], proptr],
             ...
             {propsnpls, ar} = segloglike[snpls, proptr, treels[[foc, 1]], nbp + 1, theta, epsilon];
             ar += proptreels[[foc, -1]] - treels[[foc, -1]];
             ar = heat ar +logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat] - 
               logpritree[treels, nbp, theta,prilambdamin,prilambdamax, heat];
             ar2 = logpdfjumptree[tree1,type]-logpdfjumptree[tree1,TreeTransitionTypeI[tree1,treels[[foc,2]]]];
         ...
         If[ SameCoalescentTreeQ[treels[[foc - 1, 2]], treels[[foc + 1, 2]]],
             ...
             If[ SameCoalescentTreeQ[tree2,proptr],
                 ...
                 proptreels[[foc + 1, -1]] = Log[TreeTransitionProb[proptr, tree3]];
                 {propsnpls,ar} = segloglike[snpls, proptr, s2,s3,  theta, epsilon];
                 ar+=(Total[proptreels[[foc ;; foc + 1, -1]]] +Log[proptreels[[foc, 3]]])-
                     (Total[treels[[foc ;; foc + 1, -1]]] +Log[treels[[foc, 3]]]);
                 ar = heat ar+logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax,heat]-
                         logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
...
updatetreeinsert[snpls_,treels_,theta_, epsilon_,nbp_,prilambdamin_,prilambdamax_,heat_] :=
    Module[ {foc, s1, tree1, s2, tree2, ar, ar2, props, proptr, type, ff, proptreels,propsnpls},
        ...
        If[ foc == Length[treels],
            If[ nbp - s1 < 1,
                ...
                {propsnpls,ar} = segloglike[snpls, proptr, props,nbp+1,  theta, epsilon];
            ...
            If[ s2 - s1 < 2,
                ...
                Switch[First[type],
                    ...
                    {propsnpls,ar} = segloglike[snpls, proptr, props,s2,  theta, epsilon];
                    ar+=(Total[proptreels[[foc + 1 ;; foc + 2, -1]]] +Total[Log[proptreels[[foc ;; foc + 1, 3]]]])-
                        (treels[[foc + 1, -1]] + Log[treels[[foc, 3]]]);
                    ar = heat ar+logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax,heat]-
                        logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
...
updatetreedelete[snpls_,treels_, theta_, epsilon_, nbp_,prilambdamin_,prilambdamax_,heat_] :=
    Module[ {ar,ar2,foc,s1,s2,s3,tree1,tree2,tree3,type,proptreels,propsnpls,ff},
        If[ Length[treels] == 1,
            ...
            If[ foc == Length[treels],
                ...
                {propsnpls,ar} = segloglike[snpls, tree1, s2,nbp+1,  theta, epsilon];
                ...
                ar = Min[1, Exp[ar - ar2]],
                {{s1, tree1}, {s2, tree2}, {s3, tree3}} = treels[[foc - 1 ;; foc + 1, {1, 2}]];
                type = TreeTransitionTypeII[tree1, tree3];
                (*Print["type=",type];*)
                Switch[ First[type],
                    0,
                    ...
                    {propsnpls,ar} = segloglike[snpls, tree1, s2,s3, theta, epsilon];
                    ar+=(proptreels[[foc, -1]] +Log[proptreels[[foc - 1, 3]]])-
                        (Total[treels[[foc ;; foc + 1, -1]]] +Total[Log[treels[[foc - 1 ;; foc, 3]]]]);
                    ar = heat ar+logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax,heat]-
                    logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
...
updatetreeswapch[inputsnpls_, inputtreels_, theta_, epsilon_, nsq_, nbp_, heat_] :=
...
updatetreechecker[inputsnpls_, inputtreels_, theta_, epsilon_, nbp_, prilambdamin_,prilambdamax_, heat_] :=
    Module[ {snpls = inputsnpls, treels = inputtreels, ind = {0, 10^(-6.)}, ar, ar2, rho, rg, props, proptr, proptreels,
        ...
        {snpls, treels, ar} = updatetreepostree[snpls, treels, 1, theta, epsilon, nbp, prilambdamin,prilambdamax, heat];
        ind += {ar, 1};
        {snpls, treels, ar} = updatetreepostree[snpls, treels, Length[treels], theta, epsilon, nbp,prilambdamin,prilambdamax, heat];
        ind += {ar, 1};
        Do[
            (*rho = updateRho[treels, nbp, theta,prilambdamin,prilambdamax heat];*)
            (*{rho,adprho}=updateRho[treels, rho, nbp, theta,prilambdamin,prilambdamax,heat,isadp, adprate, adprho, heat];*)
            ...
            logl = TreeLogLikelihood[snpls[[All, 2]], trees,snpls[[All, 3]], theta, epsilon];
...
blockupdatetreepos[inputsnpls_, inputtreels_, theta_, epsilon_, nbp_, prilambdamin_,prilambdamax_, heat_] :=
        ...
        Do[
         ...
         Do[
          {propsnpls, ind} = If[ treels[[i, 1]] < proptreels[[i, 1]],
                                 segloglike[propsnpls, proptreels[[i - 1, 2]], treels[[i, 1]], proptreels[[i, 1]], theta, epsilon],
                                 segloglike[propsnpls, proptreels[[i, 2]], proptreels[[i, 1]], treels[[i, 1]], theta, epsilon]
                             ];
          ar+=ind, {i, rg}];
         ar = heat ar +logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat] - 
               logpritree[treels, nbp, theta,prilambdamin,prilambdamax, heat];
...
rjupdatetreels[inputsnpls_, inputtreels_, theta_, epsilon_, nsq_,nbp_, prilambdamin_,prilambdamax_,
    heat_,size_,isadp_,adprate_, inputadpposseglen_] :=
    Module[ {adpposseglen = inputadpposseglen,snpls = inputsnpls,treels = inputtreels, ind = Table[{0, 10^(-6.)}, {10}],
        maxseglen,act,actls,ar,segar,segarlist,posseglen,istest = False,cc},
        cc = 0.35;
        ...
        *)
        ...
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"rjstart: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        segarlist = Reap[Do[
            Check[
            ...
            (*Print["bef; act=",act];*)
            Switch[act,          
               1,{snpls,treels, ar} = updatecoaltime[snpls, treels, theta, epsilon, nsq,nbp, prilambdamin,prilambdamax,heat],
               2,{snpls, treels, ar} = updatesegtreepos[snpls, treels, posseglen, theta, epsilon, nbp, prilambdamin,prilambdamax,heat],
               3,{snpls,treels, segar} = updatesegtreetree[snpls, treels, theta, epsilon, nbp, prilambdamin,prilambdamax,heat,maxseglen],
               4,{snpls,treels, segar} = updatesegtreeinsert[snpls,treels, theta, epsilon,nbp,prilambdamin,prilambdamax,heat,maxseglen],
               5,{snpls,treels, segar} = updatesegtreedelete[snpls,treels, theta, epsilon, nbp,prilambdamin,prilambdamax,heat,maxseglen]
             ];
            (*Print["after; act=",act];*)
            ...
            If[ istest,
                If[ snpls=!={},
                    testsnpls[snpls,treels,nbp,theta,epsilon,"rj: wrong in snpls[[All,-1]]!! act="<>ToString[act]]
                ];
            ];
        ...
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"rjend: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ ind[[2,2]]>=1,
        ...
        {snpls, treels, ar} = updatetreeswapch[snpls, treels, theta, epsilon, nsq, nbp, heat];
        ind[[10]]+={ar,1};
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,"swapch: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
...
randomjumptree[tree_, e0 : {_Integer, _Integer}] :=
    Module[ {tlist, elist, n, k, t0, t1, e1, newtree, newtype},
        ...
        {t1, e1} = RandomCoalescentPoint[tree, t0];
...
randomjumpcondtree[tree1_, tree2_] :=
    Module[ {n,type, e0ls, newtree, newtype, ar2,sharee0},
        ...
        ,Print["{tree1,tree2,type}=",{tree1,tree2,type}];
         {ar2, newtree} = {0,tree2}];
        {ar2, newtree}
    ]
...
condTreeTransitionTypeII[tree11_, tree1_, tree2_, type_] :=
    Module[ {type2,n, t1,temp,set},
        type2 = TreeTransitionTypeII2[tree11, tree2];
        n = LeafNumber[tree11];
        If[ type2[[2]] == 2,
            t1 = type[[-1, 1, 2]];
...
segloglike2[inputsnpls_, treels_, istart_, iend_, theta_, epsilon_, nbp_] :=
    Module[ {xstart, xend, pos, newsnpls = inputsnpls, trees,ls},
        ...
        If[ pos === {},
            ...
            newsnpls[[pos, -1]] = TreeLogLikelihood[newsnpls[[pos, 2]], trees, newsnpls[[pos, 3]],theta, epsilon];
            {newsnpls, Total[newsnpls[[pos, -1]]] - Total[inputsnpls[[pos, -1]]]}
        ]
    ]    

updatesegtreetree[snpls_, treels_, theta_, epsilon_, nbp_, prilambdamin_,prilambdamax_,heat_,maxseglen_] :=
    Module[ {foc,proptr, proptreels, propsnpls, seglen = 1, ar, ar2,type,tree1,ar2sub,propsub, maxpos},
        ...
        Which[
         ...
         If[ SameCoalescentTreeQ[treels[[foc, 2]], proptr],
             ...
             {propsnpls, ar} = segloglike[snpls, proptr, treels[[foc, 1]], nbp + 1, theta,epsilon];
             ar+= proptreels[[foc, -1]] - treels[[foc, -1]];
             ar+= logpriSNP[snpls[[All,1]],proptreels, foc,foc, theta, epsilon, nbp]-logpriSNP[snpls[[All,1]],treels, foc,foc, theta, epsilon, nbp];
             ar = heat ar +logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat] - 
               logpritree[treels, nbp, theta,prilambdamin,prilambdamax, heat];
             ar2 = logpdfjumptree[tree1,type]-logpdfjumptree[proptr,TreeTransitionTypeI[proptr,tree1]];
         ...
         If[ SameCoalescentTreeQ[treels[[foc, 2]], proptr],
             ...
             If[ ar2sub === None,
                 ...
                 ar2 += ar2sub;
                 {propsnpls, ar} = segloglike2[snpls, proptreels, foc, maxpos, theta, epsilon,nbp];
                 ...
                 ar+= logpriSNP[snpls[[All,1]],proptreels, foc,maxpos, theta, epsilon, nbp]-logpriSNP[snpls[[All,1]],treels, foc,maxpos, theta, epsilon, nbp];
                 ar = heat ar +logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat] - 
                   logpritree[treels, nbp, theta,prilambdamin,prilambdamax, heat];
                 If[ !(NumericQ[ar]&&NumericQ[ar2]),
...
randomsegtreeprop[1, snpls_, treels_, theta_, epsilon_, nbp_,maxseglen_,prilambdamin_,prilambdamax_,heat_] :=
    Module[ {tree1,type,proptr,ar2,proptreels,propsnpls,propsub,seglen,ar,ar2sub},
        ...
        If[ SameCoalescentTreeQ[tree1, proptr],
            ...
            If[ ar2sub === None,
                ...
                ar2 += ar2sub;
                {propsnpls, ar} = segloglike2[snpls, proptreels, 1, seglen, theta, epsilon,nbp];
                ar += (Total[proptreels[[;; seglen, -1]]] + Log[Times @@ proptreels[[;; seglen - 1, 3]]]) - 
                        (Total[treels[[;; seglen, -1]]] + Log[Times @@ treels[[;; seglen - 1, 3]]]);
                ar+= logpriSNP[snpls[[All,1]],proptreels, 1,seglen, theta, epsilon, nbp]-
                        logpriSNP[snpls[[All,1]],treels, 1,seglen, theta, epsilon, nbp];
                ar = heat ar +logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat]- 
                          logpritree[treels, nbp, theta,prilambdamin,prilambdamax, heat];
                ar = Min[1, Exp[ar - ar2]];
...
randomsegtreeprop[foc_/;foc>=2, snpls_, treels_, theta_, epsilon_, nbp_,maxseglen_,prirhoshape_, prirhoscale_,prirhomin_,prirhomax_,heat_] :=
        ...
        If[ SameCoalescentTreeQ[treels[[ii,2]], proptr],
            ...
            If[ ar2sub === None,
                ...
                If[ ar2sub2 === None,
                    ...
                    {propsnpls, ar} = segloglike2[snpls, proptreels, ii+1, maxpos, theta, epsilon,nbp];
                    ...
                    ar+= logpriSNP[snpls[[All,1]],proptreels, ii+1,maxpos, theta, epsilon, nbp]-
                            logpriSNP[snpls[[All,1]],treels, ii+1,maxpos, theta, epsilon, nbp];
...
updatesegtreetree2[snpls_, treels_, theta_, epsilon_, nbp_, prilambdamin_,prilambdamax_,heat_,maxseglen_] :=
    Module[ {foc,proptr, proptreels, propsnpls, seglen = 1, ar, ar2,type,tree1},
        ...
        Which[
         ...
         True,
         {ar,seglen,propsnpls,proptreels} = randomsegtreeprop[foc, snpls, treels, theta, epsilon,nbp,maxseglen,prilambdamin,prilambdamax,heat]
        ];
        If[ RandomReal[] < ar,
            {propsnpls,proptreels, {seglen, ar}},
            {snpls, treels, {seglen, ar}}
...
updatesegtreeinsert[inputsnpls_, inputtreels_, theta_, epsilon_, nbp_,prilambdamin_,prilambdamax_, heat_,maxseglen_] :=
    Module[ {snpls = inputsnpls, treels = inputtreels,seglen = 1,foc, proptr, seg0,type, 
      ar, props, proptreels, ar2, propsnpls,ar2sub,propsub, maxpos},
        ...
        Which[
            ...
            {propsnpls, ar} = segloglike[snpls, proptr, props, nbp + 1, theta, epsilon];
            ar += proptreels[[foc + 1, -1]] + Log[proptreels[[foc, 3]]];
            ar+= logpriSNP[snpls[[All,1]],proptreels, foc,foc+1, theta, epsilon, nbp]-logpriSNP[snpls[[All,1]],treels, foc,foc, theta, epsilon, nbp];
            ar = heat ar + logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat] - 
              logpritree[treels, nbp, theta,prilambdamin,prilambdamax, heat];
            ...
            If[ ar2sub === None,
                ...
                {propsnpls, ar} = segloglike2[snpls, proptreels, foc+1,  maxpos, theta, epsilon,nbp];
                ...
                ar+= logpriSNP[snpls[[All,1]],proptreels, foc,maxpos, theta, epsilon, nbp]-logpriSNP[snpls[[All,1]],treels, foc,maxpos-1, theta, epsilon, nbp];
                ar = heat ar +logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat] - 
                  logpritree[treels, nbp, theta,prilambdamin,prilambdamax, heat];
                If[ !(NumericQ[ar]&&NumericQ[ar2]),
...
updatesegtreedelete[inputsnpls_, inputtreels_, theta_, epsilon_, nbp_,prilambdamin_,prilambdamax_, heat_,maxseglen_] :=
    Module[ {snpls = inputsnpls, treels = inputtreels, seglen = 1,ar,type,
      foc, proptreels, ar2, propsub,  maxpos, propsnpls, seg0,ar2sub},
        If[ Length[treels]==1,
            ...
            If[ foc==Length[treels],
                {propsnpls,ar} = segloglike[snpls, treels[[foc-1,2]], treels[[foc,1]],nbp+1,  theta, epsilon];
                ar+=0-(treels[[foc, -1]] +Log[treels[[foc - 1, 3]]]);
                ar+= logpriSNP[snpls[[All,1]],proptreels, foc-1,foc-1, theta, epsilon, nbp]-logpriSNP[snpls[[All,1]],treels, foc-1,foc, theta, epsilon, nbp];
                ar = heat ar+logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax,heat]-
                    logpritree[treels, nbp, theta,prilambdamin,prilambdamax,heat];
                ...
                If[ ar2sub ===None,
                    ...
                    ar+= logpriSNP[snpls[[All,1]],proptreels, foc-1,maxpos, theta, epsilon, nbp]-logpriSNP[snpls[[All,1]],treels, foc-1,maxpos+1, theta, epsilon, nbp];
                    ar = heat ar + logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat] - 
                      logpritree[treels, nbp, theta,prilambdamin,prilambdamax, heat];
                    If[ !(NumericQ[ar]&&NumericQ[ar2]),
...
updatesegtreepos[snpls_, treels_, segl_, theta_, epsilon_, nbp_, prilambdamin_,prilambdamax_, heat_] :=
    Module[ {i1, i2, xleft, xright, proptreels, propsnpls, ar},
        If[ Length[treels]==1,
            ...
            {propsnpls, ar} = segloglike2[snpls, proptreels, i1-1,i2, theta, epsilon,nbp];
            ar+= logpriSNP[snpls[[All,1]],proptreels, i1-1,i2, theta, epsilon, nbp]-logpriSNP[snpls[[All,1]],treels, i1-1,i2, theta, epsilon, nbp];
            ar = heat ar + logpritree[proptreels, nbp, theta,prilambdamin,prilambdamax, heat]-
                logpritree[treels, nbp, theta,prilambdamin,prilambdamax, heat];
            If[ !(NumericQ[ar]),
                Print["1: {ar}=",{ar}]
