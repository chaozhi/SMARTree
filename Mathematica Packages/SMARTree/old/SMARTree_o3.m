...
initializeSMARTchain[inputsnpls_,nsq_, nbp_,initheta_,inirho_,inieps_,modelid_] :=
    Module[ {inithetashape, inithetascale, inithetamin, inithetamax, inirhoshape, inirhoscale, inirhomin, inirhomax, 
        ...
        adpthetaerr = InitialProposal[0.234, Log[{theta, epsilon}]];
...
saveSMARTchain[mcstate_, nsq_, adpheat_, outputconcise_, outputcold_] :=
    Module[ {mcstate2, mcstate3, i, j},
        mcstate2 = mcstate;
        Do[
         mcstate2[[i, j, 4]] = Length[mcstate2[[i, j, 4]]];
         mcstate2[[i, j, 5]] = Prepend[mcstate[[i, j, 5]], adpheat[[i, j]]], {i, Length[mcstate]}, {j, Dimensions[mcstate][[2]]}];
        mcstate2[[All, All, 5]] = mcstate2[[All, All, 5, All, {4, 5}]];
        mcstate2 = Flatten[mcstate2[[All, All, ;; -2]], 1];
        PutAppend[mcstate2, outputconcise];
        mcstate3 = First[Select[#[[All, ;; -2]], First[#] == 1 &]] & /@ mcstate;
        Do[
         mcstate3[[i, 4, All, 2]] = {#[[1, nsq ;;]], TreeToAdjacencyList[#]} & /@mcstate3[[i, 4, All, 2]];
         mcstate3[[i, 5]] = Prepend[mcstate3[[i, 5]], Mean[Most[adpheat[[i]]]]], {i,Length[mcstate3]}];
...
swapSMARTchain[inputmcstate_, isadp_, adprate_, inputadpheat_] :=
    Module[ {mcstate = inputmcstate, adpheat = inputadpheat, i,j, jj, ar},
        Do[
         jj = {j, j + 1};
         ar = mcstate[[i, Reverse[jj], 1]].mcstate[[i, jj, 2]] - 
           mcstate[[i, jj, 1]].mcstate[[i, jj, 2]];
         ar = Min[1, Exp[ar]];
         If[ RandomReal[] < ar,
             mcstate[[i, jj, 2 ;;]] = mcstate[[i, Reverse[jj], 2 ;;]];
         ];
         adpheat[[i, j]] = AdaptiveProposalScale[isadp, ar, adprate, adpheat[[i, j]]];
         If[ isadp,
             mcstate[[i, ;; j, 1]] -= adpheat[[i, j, -1]] + Subtract @@ mcstate[[i, jj, 1]];
         ], {i, Length[mcstate]}, {j, -2, -Length[mcstate[[i]]], -1}];
        {mcstate, adpheat}
    ]    
...
updateThetaEpsilon[inputsnpls_,treels_,inputtheta_, inputepsilon_, rho_,nbp_,prithetashape_, 
    ...
    Module[ {snpls = inputsnpls,adpset = inputadpset,theta = inputtheta,epsilon = inputepsilon,
        ...
        logprihyper = Function[{x},
            (prithetashape-1)x[[1]]-Exp[x[[1]]]/prithetascale + x[[1]]+
            (priepsalpha - 1) x[[2]] + (priepsbeta - 1) Log[1 - Exp[x[[2]]]]+x[[2]]
            ];
        treels2 = Append[treels, ReplacePart[treels[[-1]], 1 -> nbp + 1]];
        treemap = IndexByInterpolation[treels2[[All, 1]]];
        ytree = treels2[[treemap[snpls[[All, 1]]], 2]];
        ...
        res = Table[0, {size}];
        Do[
          propx = Log[{theta, epsilon}] + deltxls[[i]];
          If[ prithetamin<Exp[propx[[1]]]<prithetamax && priepsmin < Exp[propx[[2]]] <priepsmax,
              propsnpls = snpls;
              If[ propsnpls==={},
                  ar = 0,
                  propsnpls[[All, -1]] = TreeLogLikelihood[propsnpls[[All, 2]], ytree, propsnpls[[All, 3]],Exp[propx[[1]]], Exp[propx[[2]]],modelid];
                  ar = Total[propsnpls[[All, -1]]] - Total[snpls[[All, -1]]];
              ];
              ar += logprihyper[propx] - logprihyper[Log[{theta, epsilon}]];
              ar+= logpriSNP[snpls[[All, 1]],treels, 1, Length[treels], Exp[propx[[1]]], Exp[propx[[2]]], nbp,modelid]-
                      logpriSNP[snpls[[All, 1]],treels, 1, Length[treels], theta,epsilon, nbp,modelid];
              ar = heat ar+logpritree[treels, nbp, Exp[propx[[1]]],prirhoshape, prirhoscale,prirhomin,prirhomax,heat]-
                          logpritree[treels, nbp, theta,prirhoshape, prirhoscale,prirhomin,prirhomax,heat];
              ar = Min[1, Exp[ar]];
              If[ RandomReal[] < ar,
                  {theta, epsilon} = Exp[propx];
                  snpls = propsnpls;
              ],
              ar = 0
          ];
          res[[i]] = {ar, Log[{theta, epsilon}]}, {i, size}];
