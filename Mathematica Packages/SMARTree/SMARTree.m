...
updateSMARTchain::usage = "updateSMARTchain[chainstate,nsq,nbp, pritheta,prilambda,prieps,isadp,adprate,rjfreq,isprint] updates chainstate for one MCMC chain."
...
testsnpls::usage = "testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,msg] gives True if the log likelihoods of SNP data given the local trees (column 2 of treels) are consistent, and otherwise gives False and prints message msg."
...
initializeSMARTchain[inputsnpls_,nsq_, nbp_,initheta_,inieps_,inilam_,primodel_,ascertainmodel_] :=
    Module[ {inithetashape, inithetascale, inithetamin, inithetamax, inilamshape, inilamscale, inilammin, inilammax, 
        iniepsalpha, iniepsbeta, iniepsmin, iniepsmax,snpls = inputsnpls,theta, epsilon, rho, lam,treels, adpscale, 
        ...
        {inilamshape, inilamscale, inilammin, inilammax} = inilam;
        ...
        lam = RandomReal[If[ inilamscale == Infinity,
                             UniformDistribution[{inilammin, inilammax}],
                             TruncatedDistribution[{inilammin, inilammax},GammaDistribution[inilamshape, inilamscale]]
                         ]];
        rho = If[ primodel=="P1",
                  lam,
                  theta lam
        ...
        snpls[[All, -1]] = TreeLogLikelihood[snpls[[All, 2]], ytree, snpls[[All, 3]], theta, epsilon,ascertainmodel];
...
updateSMARTchain[chainstate_, nsq_, nbp_, pritheta_,prieps_,prilam_,primodel_,ascertainmodel_,isadp_,adprate_,rjfreq_,isprint_] :=
    Module[ {prithetashape, prithetascale, prithetamin, prithetamax, prilamshape, prilamscale, prilammin, prilammax, 
        ...
        {prilamshape, prilamscale, prilammin, prilammax} = prilam;
        ...
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"0:"<>ToString[heat]]
            ];
        ];
        If[ isprint,
            Print["update rj..."]
        ...
        {snpls,treels,adpposseglen,segarlist,indtr} = rjupdatetreels[snpls, treels, theta, epsilon, nsq,nbp, 
            prilamshape, prilamscale,prilammin,prilammax,primodel,ascertainmodel,heat, rjfreq,isadp,adprate,adpposseglen];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"1: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
            Print["update timescale..."]
        ];
        {snpls,treels, adpscale} = updatetimescale[snpls, treels, theta, epsilon, nsq, nbp,prilamshape, prilamscale,prilammin,prilammax,primodel,
            ascertainmodel,heat, isadp, adprate, adpscale];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"2: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
            Print["update timeshift..."]
        ];
        {snpls,treels, adpshift} = updateshift[snpls, treels, theta, epsilon, nsq, nbp,prilamshape, prilamscale,prilammin,prilammax,primodel, 
            ascertainmodel,heat, isadp, adprate, adpshift];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"3: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
        ...
        {snpls,{theta, epsilon}, adpthetaerr} = updateThetaEpsilon[snpls, treels, theta, epsilon,rho,nbp, 
            prithetashape,prithetascale, prithetamin,prithetamax,priepsalpha, priepsbeta,priepsmin,priepsmax,
            prilamshape, prilamscale,prilammin,prilammax,primodel,ascertainmodel,heat, isadp, adprate,adpthetaerr];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"4: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
        ...
        {snpls, treels, {theta, epsilon}, adphyperscale} = updatehyperscale[snpls, treels, theta, epsilon, nsq, nbp, 
             prilamshape, prilamscale,prilammin,prilammax, prithetashape,prithetascale,prithetamin, prithetamax,priepsalpha, priepsbeta, 
             priepsmin,priepsmax,primodel,ascertainmodel,heat, isadp, adprate, adphyperscale];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"5: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
        ...
        {snpls, treels, {theta, epsilon}, adphypershift} = updatehypershift[snpls, treels, theta, epsilon, nsq, nbp, 
            prilamshape, prilamscale,prilammin,prilammax, prithetashape,prithetascale,prithetamin, prithetamax,priepsalpha, priepsbeta, priepsmin,priepsmax,
            primodel,ascertainmodel,heat, isadp, adprate, adphypershift];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"6: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ isprint,
            Print["update rho..."];
        ];  
        (*rho = updateRho[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax,heat];*)
        {rho,adprho} = updateRho[treels, rho, nbp, theta,prilamshape, prilamscale,prilammin,prilammax,primodel,heat,isadp, adprate, adprho, heat];
        If[ isprint,
        ...
        logl+=(priepsalpha-1) Log[epsilon]+(priepsbeta-1) Log[1-epsilon]+
                ...
                (prilamshape-1)Log[rho]-rho/(prilamscale If[ primodel=="P1",
                                                             1,
                                                             theta
                                                         ])-prilamshape Log[theta];
        deltd = Append[Differences[treels[[All, 1]]], nbp - treels[[-1, 1]]];
        logl+=(Length[treels] - 1) Log[rho/2]-(deltd.treels[[All, 3]]) rho/2;
        logl+=logpriSNP[snpls[[All, 1]],treels,1, Length[treels], theta,epsilon, nbp,ascertainmodel];
        If[ direct == -1,
            {snpls, treels} = reversechromosome[snpls, treels, nbp]
        ];
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"7: "<>ToString[heat]]
            ];
        ];
...
permuteSMARTchain[inputmcstate_, isadp_, adprate_, inputadpset_,size_] :=
...
swapSMARTchain[inputmcstate_, isadp_, adprate_,inputadpset_,size_] :=
...
testsnpls[snpls_,treels_,nbp_,theta_,epsilon_,ascertainmodel_,msg_] :=
    Module[ {ls,trpos,temp},
        ls = Append[treels[[All, 1]], nbp + 1];
        trpos = IndexByInterpolation[ls][snpls[[All, 1]]];
        temp = TreeLogLikelihood[snpls[[All, 2]], treels[[trpos, 2]],snpls[[All, 3]], theta, epsilon,ascertainmodel];
...
updateThetaEpsilon[inputsnpls_,treels_,inputtheta_, inputepsilon_, rho_,nbp_,prithetashape_, 
    ...
    prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,ascertainmodel_,heat_,isadpt_, adprate_, inputadpset_] :=
    Module[ {snpls = inputsnpls,adpset = inputadpset,theta = inputtheta,epsilon = inputepsilon,
        ...
        Do[
          ...
          If[ prithetamin<propx[[1]]<prithetamax && priepsmin < propx[[2]] <priepsmax,
              ...
              If[ propsnpls==={},
                  ar = 0,
                  propsnpls[[All, -1]] = TreeLogLikelihood[propsnpls[[All, 2]], ytree, propsnpls[[All, 3]],propx[[1]], propx[[2]],ascertainmodel];
                  ar = Total[propsnpls[[All, -1]]] - Total[snpls[[All, -1]]];
              ];
              ar += logprihyper[propx] - logprihyper[{theta, epsilon}];
              ar+= logpriSNP[snpls[[All, 1]],treels, 1, Length[treels], propx[[1]], propx[[2]], nbp,ascertainmodel]-
                      logpriSNP[snpls[[All, 1]],treels, 1, Length[treels], theta,epsilon, nbp,ascertainmodel];
              ar = heat ar+logpritree[treels, nbp, propx[[1]],prilamshape, prilamscale,prilammin,prilammax,primodel,heat]-
                          logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax,primodel,heat];
              ar = Min[1, Exp[ar]];
              If[ RandomReal[] < ar,
                  {theta, epsilon} = propx;
                  snpls = propsnpls;
...
updateRho[treels_, rho_, nbp_, theta_,prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,heat_,isadp_, adprate_, adprho_, heat_] :=
    Module[ {deltd,a,b,logpdf, res,min,max},
        {min,max} = {prilammin,prilammax};
        If[ primodel=="P2",
            {min,max}*=theta
        ];
        deltd = Append[Differences[treels[[All, 1]]], nbp - treels[[-1, 1]]];
        a = heat (Length[treels] + prilamshape-2);
        b = heat (1/prilamscale + Total[deltd treels[[All, 3]]]/2);
        If[ prilammin==0&&prilammax==Infinity,
            {RandomReal[GammaDistribution[a+1,1/b]],adprho},
            logpdf = Function[{x}, a Log[x] - x b];
            res = AdaptiveMetropolis[logpdf, rho, 1, 1, Boole[isadp], adprate, adprho, DomainConstraint -> (min<#<max&)];
            res[[{1, -1}]]
        ]
    ]    
    
    
logpritree[treels_, nbp_, theta_,prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,heat_] :=
    Module[ {priscale,deltd, a,b,res},
        priscale = If[ primodel=="P1",
                       prilamscale,
                       prilamscale* theta
                   ];
        deltd = Append[Differences[treels[[All, 1]]], nbp-treels[[-1, 1]]];
        a = heat (Length[treels] + prilamshape-2);
        b = heat (1/priscale + Total[deltd treels[[All, 3]]]/2);
        res = -heat (Length[treels]-1) Log[2.]-(a+1) Log[b];
        res+Which[
            prilammin==0&&prilammax==Infinity,LogGamma[a+1],
            prilammax> (a+1)/b,
            Log[Gamma[a+1]-Gamma[a+1,prilammax b]-Gamma[a+1,0,prilammin b]],
            True,
            Log[Gamma[a+1,prilammin b,prilammax b]]
        ]
    ]          
...
logpriSNP[snpx_,treels_, istart_, iend_, theta_, epsilon_, nbp_, ascertainmodel_] :=
    Module[ {dls, forest,nmac},
        If[ snpx==={}||ascertainmodel=="M2"||ascertainmodel=="M2S",
            0,
            Switch[ascertainmodel,
            	"M1",nmac = 2,
            	"M1S",nmac = 1,
                _, Print["logpriSNP: Wrong ascertainmodel=",ascertainmodel]; Abort[]
            ];
            ...
            dls.Log[1-PolymorphicProb[forest,theta,epsilon,nmac]]
        ]
    ]

updatetimescale[snpls_,treels_, theta_, epsilon_,nsq_,nbp_,prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,ascertainmodel_,
    ...
    Module[ {dim,props,proptreels,propsnpls,ls,map,trees,ar,adpset = inputadpset},
        ...
        If[ propsnpls==={},
            ...
            propsnpls[[All, -1]] = TreeLogLikelihood[propsnpls[[All, 2]], trees, propsnpls[[All, 3]],theta, epsilon,ascertainmodel];
            ar = Total[propsnpls[[All, -1]]] - Total[snpls[[All, -1]]];
        ];
        ar += (Total[proptreels[[All, -1]]] + Log[Times @@ proptreels[[;; -2, 3]]])-
                (Total[treels[[All, -1]]] + Log[Times @@ treels[[;; -2, 3]]]);
        ar+= logpriSNP[snpls[[All, 1]],proptreels, 1,Length[proptreels], theta, epsilon, nbp,ascertainmodel]-
                logpriSNP[snpls[[All, 1]],treels, 1,Length[treels], theta, epsilon, nbp,ascertainmodel];
        ar = heat ar+logpritree[proptreels, nbp, theta, prilamshape, prilamscale,prilammin,prilammax, primodel,heat]-
                logpritree[treels, nbp, theta, prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
...
updateshift[snpls_, treels_, theta_, epsilon_, nsq_, nbp_,prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,ascertainmodel_,
  heat_, isadp_, adprate_, inputadpset_] :=
...
updatehyperscale[inputsnpls_, inputtreels_, inputtheta_, 
   inputepsilon_, nsq_,nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_,prithetashape_,
   prithetascale_,prithetamin_,prithetamax_, priepsalpha_, priepsbeta_,priepsmin_, priepsmax_,
   primodel_,ascertainmodel_, heat_, isadpt_, adprate_, inputadpset_] :=
         ...
         If[ prithetamin<propx[[2]] <prithetamax &&priepsmin < propx[[-1]] < priepsmax,
             ...
             If[ propsnpls === {},
                 ...
                 propsnpls[[All, -1]] = TreeLogLikelihood[propsnpls[[All, 2]], proptreels[[trpos, 2]],
                     propsnpls[[All, 3]], propx[[2]], propx[[3]],ascertainmodel];
                 ar = Total[propsnpls[[All, -1]]] - Total[snpls[[All, -1]]];
             ];
             ar += (Total[proptreels[[All, -1]]] +Log[Times @@ proptreels[[;; -2, 3]]])-
                     (Total[treels[[All, -1]]] + Log[Times @@ treels[[;; -2, 3]]]);
             ar += logprihyper[propx] - logprihyper[{logmtbh, theta, epsilon}];
             ar+= logpriSNP[snpls[[All, 1]],proptreels, 1,Length[proptreels], propx[[2]],propx[[3]], nbp,ascertainmodel]-
                     logpriSNP[snpls[[All, 1]],treels, 1,Length[treels], theta, epsilon, nbp,ascertainmodel];
             ar = heat ar+logpritree[proptreels, nbp, propx[[2]],prilamshape, prilamscale,prilammin,prilammax, primodel,heat]-
                     logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
...
segloglike[inputsnpls_, tree_, xstart_, xend_, theta_, epsilon_,ascertainmodel_] :=
    Module[ {pos, newsnpls = inputsnpls},
        ...
        If[ pos === {},
            ...
            newsnpls[[pos, -1]] = TreeLogLikelihood[newsnpls[[pos, 2]], Table[tree, {Length[pos]}], 
              newsnpls[[pos, 3]], theta, epsilon,ascertainmodel];
...
updatecoaltime[snpls_, treels_, theta_, epsilon_, nsq_, nbp_,prilamshape_, prilamscale_,prilammin_,prilammax_, primodel_,ascertainmodel_,heat_] :=
    Module[ {ct,temp,minpos,maxpos,coalt2,propct,proptreels,propsnpls,ar,ar2},
        ...
        {propsnpls, ar} = segloglike2[snpls, proptreels, minpos, maxpos, theta, epsilon,nbp,ascertainmodel];
        ...
        ar+= logpriSNP[snpls[[All, 1]],proptreels, minpos, maxpos, theta, epsilon, nbp,ascertainmodel]-
                logpriSNP[snpls[[All, 1]],treels, minpos, maxpos, theta, epsilon, nbp,ascertainmodel];
        ar = heat ar +logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat]-
            logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
        ar2 = If[ ct == coalt2[[-2]],
                  (-2 propct) - (-2 ct),
...
updatetreepos[snpls_,treels_, foc_,theta_, epsilon_, nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_,ascertainmodel_,heat_] :=
    Module[ {ar,s1, s2, s3, props, proptreels,propsnpls,tree1,tree2},
        If[ Length[treels] == 1,
            ...
            {propsnpls,ar} = If[ s2 < props,
                                 segloglike[snpls, tree1, s2, props, theta, epsilon,ascertainmodel],
                                 segloglike[snpls, tree2, props,s2, theta, epsilon,ascertainmodel]
                             ];
            ar = heat ar+logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat] - 
               logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
            ar = Min[1, Exp[ar]]
...
updatetreepostree[snpls_,treels_,foc_,theta_, epsilon_, nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_,ascertainmodel_,heat_] :=
    Module[ {s1, tree1, proptr, proptreels,propsnpls,ar, ar2, s2, tree2, s3,tree3, type, ff,direct,xleft,xright,
        ...
        Which[
         ...
         {propsnpls,ar} = segloglike[snpls, proptr, s1, nbp + 1, theta, epsilon,ascertainmodel];
         ar = heat ar+ logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat]-
                 logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
         ...
         If[ SameCoalescentTreeQ[tree1,proptr],
             ...
             {propsnpls,ar} = segloglike[snpls, proptr, s1,s2,  theta, epsilon,ascertainmodel];
         ...
         If[ props2<=s2,
             If[ !SameCoalescentTreeQ[tree2,proptr],
                 {propsnpls,ar} = segloglike[propsnpls,proptr,props2,nbp+1,theta,epsilon,ascertainmodel],
                 {propsnpls,ar} = segloglike[propsnpls,proptr,props2,s2,theta,epsilon,ascertainmodel]
             ],
             {propsnpls,ar} = segloglike[propsnpls,tree1,s2,props2,theta,epsilon,ascertainmodel];
             If[ !SameCoalescentTreeQ[tree2,proptr],
                 {propsnpls,temp} = segloglike[propsnpls,proptr,props2,nbp+1,theta,epsilon,ascertainmodel];
                 ar+=temp;
             ];
         ];
         ...
         If[ SameCoalescentTreeQ[treels[[foc - 1, 2]], treels[[foc + 1, 2]]],
             ...
             propsnpls[[pos, -1]] = TreeLogLikelihood[propsnpls[[pos, 2]], trees, propsnpls[[pos, 3]],theta, epsilon,ascertainmodel];
             ...
             If[ props2<=s2,
                 If[ !SameCoalescentTreeQ[tree2,proptr],
                     {propsnpls,ar} = segloglike[propsnpls,proptr,props2,s3,theta,epsilon,ascertainmodel],
                     {propsnpls,ar} = segloglike[propsnpls,proptr,props2,s2,theta,epsilon,ascertainmodel]
                 ],
                 {propsnpls,ar} = segloglike[propsnpls,tree1,s2,props2,theta,epsilon,ascertainmodel];
                 If[ !SameCoalescentTreeQ[tree2,proptr],
                     {propsnpls,temp} = segloglike[propsnpls,proptr,props2,s3,theta,epsilon,ascertainmodel];
                     ar+=temp;
                 ];
             ];
             ar+=(Total[proptreels[[foc ;; foc + 1, -1]]] +Log[proptreels[[foc, 3]]])-
...
updatetreetree[snpls_,treels_,foc_,theta_, epsilon_, nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_,ascertainmodel_,heat_] :=
    Module[ {s1, tree1, proptr, proptreels,propsnpls,ar, ar2, s2, tree2, s3,tree3, type, ff,direct,xleft,xright,i2min,
        ...
        Which[
         ...
         If[ SameCoalescentTreeQ[treels[[1, 2]], proptr],
             ...
             proptreels = treels;
             proptreels[[1, 2 ;;]] = {proptr, TotalBranchHeight[proptr], TreeLogPriorProb[proptr]};
             {propsnpls, ar} = segloglike[snpls, proptr, treels[[1, 1]], nbp + 1, theta,epsilon,ascertainmodel];
             ar = heat ar +logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat] - 
               logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
         ...
         If[ SameCoalescentTreeQ[treels[[foc,2]],proptr],
             ...
             {propsnpls,ar} = segloglike[snpls, proptr, treels[[foc,1]],treels[[foc+1,1]],  theta, epsilon,ascertainmodel];
             ar += (Total[proptreels[[foc ;; foc + 1, -1]]] + Log[proptreels[[foc, 3]]])-
                  (Total[treels[[foc ;; foc + 1, -1]]] +Log[treels[[foc, 3]]]);
             ar = heat ar+logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat]-
                     logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
         ...
         If[ SameCoalescentTreeQ[treels[[foc, 2]], proptr],
             ...
             {propsnpls, ar} = segloglike[snpls, proptr, treels[[foc, 1]], nbp + 1, theta, epsilon,ascertainmodel];
             ar += proptreels[[foc, -1]] - treels[[foc, -1]];
             ar = heat ar +logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat] - 
               logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
             ar2 = logpdfjumptree[tree1,type]-logpdfjumptree[tree1,TreeTransitionTypeI[tree1,treels[[foc,2]]]];
         ...
         If[ SameCoalescentTreeQ[treels[[foc - 1, 2]], treels[[foc + 1, 2]]],
             ...
             If[ SameCoalescentTreeQ[tree2,proptr],
                 ...
                 proptreels[[foc + 1, -1]] = Log[TreeTransitionProb[proptr, tree3]];
                 {propsnpls,ar} = segloglike[snpls, proptr, s2,s3,  theta, epsilon,ascertainmodel];
                 ar+=(Total[proptreels[[foc ;; foc + 1, -1]]] +Log[proptreels[[foc, 3]]])-
                     (Total[treels[[foc ;; foc + 1, -1]]] +Log[treels[[foc, 3]]]);
                 ar = heat ar+logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat]-
                         logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
...
updatetreeinsert[snpls_,treels_,theta_, epsilon_,nbp_,prilamshape_, prilamscale_,prilammin_,prilammax_,ascertainmodel_,heat_] :=
    Module[ {foc, s1, tree1, s2, tree2, ar, ar2, props, proptr, type, ff, proptreels,propsnpls},
        ...
        If[ foc == Length[treels],
            If[ nbp - s1 < 1,
                ...
                {propsnpls,ar} = segloglike[snpls, proptr, props,nbp+1,  theta, epsilon,ascertainmodel];
            ...
            If[ s2 - s1 < 2,
                ...
                Switch[First[type],
                    ...
                    {propsnpls,ar} = segloglike[snpls, proptr, props,s2,  theta, epsilon,ascertainmodel];
                    ar+=(Total[proptreels[[foc + 1 ;; foc + 2, -1]]] +Total[Log[proptreels[[foc ;; foc + 1, 3]]]])-
                        (treels[[foc + 1, -1]] + Log[treels[[foc, 3]]]);
                    ar = heat ar+logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat]-
                        logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
...
updatetreedelete[snpls_,treels_, theta_, epsilon_, nbp_,prilamshape_, prilamscale_,prilammin_,prilammax_,ascertainmodel_,heat_] :=
    Module[ {ar,ar2,foc,s1,s2,s3,tree1,tree2,tree3,type,proptreels,propsnpls,ff},
        If[ Length[treels] == 1,
            ...
            If[ foc == Length[treels],
                ...
                {propsnpls,ar} = segloglike[snpls, tree1, s2,nbp+1,  theta, epsilon,ascertainmodel];
                ...
                Switch[ First[type],
                    ...
                    {propsnpls,ar} = segloglike[snpls, tree1, s2,s3, theta, epsilon,ascertainmodel];
                    ar+=(proptreels[[foc, -1]] +Log[proptreels[[foc - 1, 3]]])-
                        (Total[treels[[foc ;; foc + 1, -1]]] +Total[Log[treels[[foc - 1 ;; foc, 3]]]]);
                    ar = heat ar+logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat]-
                    logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
...
updatetreeswapch[inputsnpls_, inputtreels_, theta_, epsilon_, nsq_, nbp_, ascertainmodel_,heat_] :=
          ...
          If[ propsnpls === {},
              ...
              If[ pos === {},
                  ...
                  propsnpls[[pos, -1]] = TreeLogLikelihood[propsnpls[[pos, 2]],proptreels[[trpos[[pos]], 2]], 
                      propsnpls[[pos, 3]], theta,epsilon,ascertainmodel];
                  ar = Total[propsnpls[[pos, -1]]] - Total[snpls[[pos, -1]]];
...
updatetreechecker[inputsnpls_, inputtreels_, theta_, epsilon_, nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_, ascertainmodel_,heat_] :=
    Module[ {snpls = inputsnpls, treels = inputtreels, ind = {0, 10^(-6.)}, ar, ar2, rho, rg, props, proptr, proptreels,
        ...
        {snpls, treels, ar} = updatetreepostree[snpls, treels, 1, theta, epsilon, nbp, prilamshape, prilamscale,prilammin,prilammax, heat];
        ind += {ar, 1};
        {snpls, treels, ar} = updatetreepostree[snpls, treels, Length[treels], theta, epsilon, nbp,prilamshape, prilamscale,prilammin,prilammax, heat];
        ind += {ar, 1};
        Do[
            ...
            logl = TreeLogLikelihood[snpls[[All, 2]], trees,snpls[[All, 3]], theta, epsilon,ascertainmodel];
...
blockupdatetreepos[inputsnpls_, inputtreels_, theta_, epsilon_, nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_, ascertainmodel_,heat_] :=
        ...
        Do[
         ...
         Do[
          {propsnpls, ind} = If[ treels[[i, 1]] < proptreels[[i, 1]],
                                 segloglike[propsnpls, proptreels[[i - 1, 2]], treels[[i, 1]], proptreels[[i, 1]], theta, epsilon,ascertainmodel],
                                 segloglike[propsnpls, proptreels[[i, 2]], proptreels[[i, 1]], treels[[i, 1]], theta, epsilon,ascertainmodel]
                             ];
          ar+=ind, {i, rg}];
         ar = heat ar +logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat] - 
               logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
...
rjupdatetreels[inputsnpls_, inputtreels_, theta_, epsilon_, nsq_,nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_,
    primodel_,ascertainmodel_,heat_,size_,isadp_,adprate_, inputadpposseglen_] :=
    Module[ {adpposseglen = inputadpposseglen,snpls = inputsnpls,treels = inputtreels, ind = Table[{0, 10^(-6.)}, {10}],
        ...
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"rjstart: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        segarlist = Reap[Do[
            Check[
            ...
            (*Print["bef; act=",act];*)
            Switch[act,          
               1,{snpls,treels, ar} = updatecoaltime[snpls, treels, theta, epsilon, nsq,nbp, prilamshape, prilamscale,prilammin,prilammax,primodel,ascertainmodel,heat],
               2,{snpls, treels, ar} = updatesegtreepos[snpls, treels, posseglen, theta, epsilon, nbp, prilamshape, prilamscale,prilammin,prilammax,primodel,ascertainmodel,heat],
               3,{snpls,treels, segar} = updatesegtreetree[snpls, treels, theta, epsilon, nbp, prilamshape, prilamscale,prilammin,prilammax,primodel,ascertainmodel,heat,maxseglen],
               4,{snpls,treels, segar} = updatesegtreeinsert[snpls,treels, theta, epsilon,nbp,prilamshape, prilamscale,prilammin,prilammax,primodel,ascertainmodel,heat,maxseglen],
               5,{snpls,treels, segar} = updatesegtreedelete[snpls,treels, theta, epsilon, nbp,prilamshape, prilamscale,prilammin,prilammax,primodel,ascertainmodel,heat,maxseglen]
             ];
            (*Print["after; act=",act];*)
            ...
            If[ istest,
                If[ snpls=!={},
                    testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"rj: wrong in snpls[[All,-1]]!! act="<>ToString[act]]
                ];
            ];
        ...
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"rjend: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
        If[ ind[[2,2]]>=1,
        ...
        {snpls, treels, ar} = updatetreeswapch[snpls, treels, theta, epsilon, nsq, nbp, ascertainmodel,heat];
        ind[[10]]+={ar,1};
        If[ istest,
            If[ snpls=!={},
                testsnpls[snpls,treels,nbp,theta,epsilon,ascertainmodel,"swapch: wrong in snpls[[All,-1]]!!"<>ToString[heat]]
            ];
        ];
...
segloglike2[inputsnpls_, treels_, istart_, iend_, theta_, epsilon_, nbp_,ascertainmodel_] :=
    Module[ {xstart, xend, pos, newsnpls = inputsnpls, trees,ls},
        ...
        If[ pos === {},
            ...
            newsnpls[[pos, -1]] = TreeLogLikelihood[newsnpls[[pos, 2]], trees, newsnpls[[pos, 3]],theta, epsilon,ascertainmodel];
            {newsnpls, Total[newsnpls[[pos, -1]]] - Total[inputsnpls[[pos, -1]]]}
        ]
    ]    

randomsegtreeprop[1, snpls_, treels_, theta_, epsilon_, nbp_,maxseglen_,prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,ascertainmodel_,heat_] :=
    Module[ {tree1,type,proptr,ar2,proptreels,propsnpls,propsub,seglen,ar,ar2sub},
        ...
        If[ SameCoalescentTreeQ[tree1, proptr],
            ...
            If[ ar2sub === None,
                ...
                ar2 += ar2sub;
                {propsnpls, ar} = segloglike2[snpls, proptreels, 1, seglen, theta, epsilon,nbp,ascertainmodel];
                ar += (Total[proptreels[[;; seglen, -1]]] + Log[Times @@ proptreels[[;; seglen - 1, 3]]]) - 
                        (Total[treels[[;; seglen, -1]]] + Log[Times @@ treels[[;; seglen - 1, 3]]]);
                ar+= logpriSNP[snpls[[All,1]],proptreels, 1,seglen, theta, epsilon, nbp,ascertainmodel]-
                        logpriSNP[snpls[[All,1]],treels, 1,seglen, theta, epsilon, nbp,ascertainmodel];
                ar = heat ar +logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat]- 
                          logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
                ar = Min[1, Exp[ar - ar2]];
...
randomsegtreeprop[foc_/;foc>=2, snpls_, treels_, theta_, epsilon_, nbp_,maxseglen_,prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,ascertainmodel_,heat_] :=
        ...
        If[ SameCoalescentTreeQ[treels[[ii,2]], proptr],
            ...
            If[ ar2sub === None,
                ...
                If[ ar2sub2 === None,
                    ...
                    {propsnpls, ar} = segloglike2[snpls, proptreels, ii+1, maxpos, theta, epsilon,nbp,ascertainmodel];
                    ...
                    ar+= logpriSNP[snpls[[All,1]],proptreels, ii+1,maxpos, theta, epsilon, nbp,ascertainmodel]-
                            logpriSNP[snpls[[All,1]],treels, ii+1,maxpos, theta, epsilon, nbp,ascertainmodel];
                    ar = heat ar +logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat] - 
                      logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
                    ar = Min[1, Exp[ar - ar2]];
                    {ar, seglen, propsnpls,proptreels}
...
updatesegtreetree2[snpls_, treels_, theta_, epsilon_, nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,ascertainmodel_,heat_,maxseglen_] :=
    Module[ {foc,proptr, proptreels, propsnpls, seglen = 1, ar, ar2,type,tree1},
        ...
        Which[
         ...
         If[ SameCoalescentTreeQ[treels[[foc, 2]], proptr],
             ...
             {propsnpls, ar} = segloglike[snpls, proptr, treels[[foc, 1]], nbp + 1, theta,epsilon,ascertainmodel];
             ar+= proptreels[[foc, -1]] - treels[[foc, -1]];
             ar+= logpriSNP[snpls[[All,1]],proptreels, foc,foc, theta, epsilon, nbp,ascertainmodel]-
                     logpriSNP[snpls[[All,1]],treels, foc,foc, theta, epsilon, nbp,ascertainmodel];
             ar = heat ar +logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat] - 
               logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
             ar2 = logpdfjumptree[tree1,type]-logpdfjumptree[proptr,TreeTransitionTypeI[proptr,tree1]];
         ...
         True,
         {ar,seglen,propsnpls,proptreels} = randomsegtreeprop[foc, snpls, treels, theta, epsilon,nbp,maxseglen,prilamshape, prilamscale,prilammin,prilammax,primodel,ascertainmodel,heat]
        ];
        If[ RandomReal[] < ar,
...
updatesegtreetree[snpls_, treels_, theta_, epsilon_, nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,ascertainmodel_,heat_,maxseglen_] :=
        ...
        Which[
         ...
         If[ SameCoalescentTreeQ[treels[[foc, 2]], proptr],
             ...
             If[ ar2sub === None,
                 ...
                 ar2 += ar2sub;
                 {propsnpls, ar} = segloglike2[snpls, proptreels, foc, maxpos, theta, epsilon,nbp,ascertainmodel];
                 ...
                 ar+= logpriSNP[snpls[[All,1]],proptreels, foc,maxpos, theta, epsilon, nbp,ascertainmodel]-
                         logpriSNP[snpls[[All,1]],treels, foc,maxpos, theta, epsilon, nbp,ascertainmodel];
                 ar = heat ar +logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat] - 
                   logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
                 If[ !(NumericQ[ar]&&NumericQ[ar2]),
...
updatesegtreeinsert[inputsnpls_, inputtreels_, theta_, epsilon_, nbp_,prilamshape_, prilamscale_,prilammin_,prilammax_, primodel_,ascertainmodel_,heat_,maxseglen_] :=
    Module[ {snpls = inputsnpls, treels = inputtreels,seglen = 1,foc, proptr, seg0,type, 
      ar, props, proptreels, ar2, propsnpls,ar2sub,propsub, maxpos},
        ...
        Which[
            ...
            {propsnpls, ar} = segloglike[snpls, proptr, props, nbp + 1, theta, epsilon,ascertainmodel];
            ar += proptreels[[foc + 1, -1]] + Log[proptreels[[foc, 3]]];
            ar+= logpriSNP[snpls[[All,1]],proptreels, foc,foc+1, theta, epsilon, nbp,ascertainmodel]-
                    logpriSNP[snpls[[All,1]],treels, foc,foc, theta, epsilon, nbp,ascertainmodel];
            ar = heat ar + logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat] - 
              logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
            ...
            If[ ar2sub === None,
                ...
                {propsnpls, ar} = segloglike2[snpls, proptreels, foc+1,  maxpos, theta, epsilon,nbp,ascertainmodel];
                ...
                ar+= logpriSNP[snpls[[All,1]],proptreels, foc,maxpos, theta, epsilon, nbp,ascertainmodel]-
                        logpriSNP[snpls[[All,1]],treels, foc,maxpos-1, theta, epsilon, nbp,ascertainmodel];
                ar = heat ar +logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat] - 
                  logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
                If[ !(NumericQ[ar]&&NumericQ[ar2]),
...
updatesegtreedelete[inputsnpls_, inputtreels_, theta_, epsilon_, nbp_,prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,ascertainmodel_, heat_,maxseglen_] :=
    Module[ {snpls = inputsnpls, treels = inputtreels, seglen = 1,ar,type,
      foc, proptreels, ar2, propsub,  maxpos, propsnpls, seg0,ar2sub},
        If[ Length[treels]==1,
            ...
            If[ foc==Length[treels],
                {propsnpls,ar} = segloglike[snpls, treels[[foc-1,2]], treels[[foc,1]],nbp+1,  theta, epsilon,ascertainmodel];
                ar+=0-(treels[[foc, -1]] +Log[treels[[foc - 1, 3]]]);
                ar+= logpriSNP[snpls[[All,1]],proptreels, foc-1,foc-1, theta, epsilon, nbp,ascertainmodel]-
                        logpriSNP[snpls[[All,1]],treels, foc-1,foc, theta, epsilon, nbp,ascertainmodel];
                ar = heat ar+logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat]-
                    logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel,heat];
                ...
                If[ ar2sub ===None,
                    ...
                    ar+= logpriSNP[snpls[[All,1]],proptreels, foc-1,maxpos, theta, epsilon, nbp,ascertainmodel]-
                            logpriSNP[snpls[[All,1]],treels, foc-1,maxpos+1, theta, epsilon, nbp,ascertainmodel];
                    ar = heat ar + logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat] - 
                      logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
                    If[ !(NumericQ[ar]&&NumericQ[ar2]),
...
updatesegtreepos[snpls_, treels_, segl_, theta_, epsilon_, nbp_, prilamshape_, prilamscale_,prilammin_,prilammax_,primodel_,ascertainmodel_,heat_] :=
    Module[ {i1, i2, xleft, xright, proptreels, propsnpls, ar},
        If[ Length[treels]==1,
            ...
            {propsnpls, ar} = segloglike2[snpls, proptreels, i1-1,i2, theta, epsilon,nbp,ascertainmodel];
            ar+= logpriSNP[snpls[[All,1]],proptreels, i1-1,i2, theta, epsilon, nbp,ascertainmodel]-
                    logpriSNP[snpls[[All,1]],treels, i1-1,i2, theta, epsilon, nbp,ascertainmodel];
            ar = heat ar + logpritree[proptreels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat]-
                logpritree[treels, nbp, theta,prilamshape, prilamscale,prilammin,prilammax, primodel, heat];
            If[ !(NumericQ[ar]),
                Print["1: {ar}=",{ar}]
